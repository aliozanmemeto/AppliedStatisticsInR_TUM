...
calculate_predictors_mse <- function(data, params, predictor_col, mean_col, response_col) {
  ...
  for (county in counties) {
    ...
    # Calculate MSEs
    MSE_mu_0 <- (1 - 0)^2 * params$sigma_v + (0^2 * params$sigma_e / n_i) + 2*(1 - 0)*(bar_x_ip - 0*bar_x_i) *
      params$V_beta[2, 2] * bar_x_i + (bar_x_ip - 0*bar_x_i) * params$V_beta[2, 2] * (bar_x_ip - 0*bar_x_i)
    MSE_mu_1 <- (1 - 1)^2 * params$sigma_v + (1^2 * params$sigma_e / n_i) + 2*(1 - 1)*(bar_x_ip - 1*bar_x_i) *
      params$V_beta[2, 2] * bar_x_i + (bar_x_ip - 1*bar_x_i) * params$V_beta[2, 2] * (bar_x_ip - 1*bar_x_i)
    MSE_mu_gamma <- (1 - gamma_i)^2 * params$sigma_v + (gamma_i^2 * params$sigma_e / n_i) + 2*(1 - gamma_i)*(bar_x_ip - gamma_i*bar_x_i) *
      params$V_beta[2, 2] * bar_x_i + (bar_x_ip - gamma_i*bar_x_i) * params$V_beta[2, 2] * (bar_x_ip - gamma_i*bar_x_i)
...
list(corn = results_corn, soybeans = results_soy)
...
[2] McCullagh & Nelder, Generalized Linear Models, 1989