...
plot(ecdf.good.rs$sorted,ecdf.good.rs$ecdf, type="s", main = "ECDFs of Residual Sugar")

ecdf.bad.rs <- ecdf.func(bad.wine$residual.sugar)
plot(ecdf.bad.rs$sorted,ecdf.bad.rs$ecdf, type="s", col='brown'
     , lty=2)
...
ecdf.good.va <- ecdf.func(bad.wine$volatile.acidity)
plot(ecdf.good.va$sorted, ecdf.good.va$ecdf, type="s", main = "ECDFs of Volatile Acidity")

ecdf.bad.va <- ecdf.func(bad.wine$volatile.acidity)
plot(ecdf.bad.va$sorted, ecdf.bad.va$ecdf, type="s", col='brown'
     , lty=2)
legend("bottomright", legend = c("Good Quality", "Bad Quality"),
       col = c("black", "brown"), lty = c(1, 2))
...
ecdf.bad.ph.ks <- ecdf.ksbands(good.wine$pH)
...
```{r squared.lm}
kc_house_data <- kc_house_data %>%
  mutate(yr_built_sq = yr_built^2,
         sqft_living_sq = sqft_living^2,
         # Extended additional variables
         floors_sq = floors^2,
         grade_sq = grade^2)

# Fit the new model with squared terms
model_log_price_sq <- lm(log_price ~ bedrooms + bathrooms + sqft_living + floors + view + condition + grade + yr_built + yr_built_sq + sqft_living_sq, data = kc_house_data)
...
model_log_price_sq_train <- lm(log_price ~ bedrooms + bathrooms + sqft_living + floors + view + condition + grade + yr_built + yr_built_sq + sqft_living_sq, data = training_set)
...
model_log_price_new <- lm(log_price ~ bedrooms +  bedrooms : bathrooms  +  bathrooms : grade + sqft_living +sqft_living:floors + grade + floors  + condition + yr_built + yr_built_sq + sqft_living_sq +view + view:bathrooms  + floors_sq +grade_sq, data = training_set)
