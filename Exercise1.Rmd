...
Good wine and all wine datasets seem to have a lighter left tail and heavier right tail than the theoretical normal distribution. Bad wine seems to only have lighter right tail but left tail seems to be more or less similar to the normal distribution.

```{r ppplot.theoretical}

standardized.data <- (good.wine$pH - mean(good.wine$pH)) / sd(good.wine$pH)
probDist <- pnorm(standardized.data)
plot(ppoints(length(standardized.data)), sort(probDist),
...
Kolmogorov-Smirnov bandwidth for sample size over 35 with 0.05 alpha value is 1.36/sqrt(N) = 0.02382667. The problem with pointwise confidence intervals is, that if we compute them for a lot of points, we accumulate errors. Uniform confidence bands are slightly wider. 
...
Take the derivative with respect to the parameter μ:
\[\frac{\partial l}{\partial \mu} = -\frac{\sqrt{2}}{\sigma} \sum_{t=1}^{N} \frac{\partial |x_t-\mu|}{\partial \mu}\]

which is equal to:
= \[\frac{\sqrt{2}}{\sigma} \sum_{t=1}^{N} \text{sgn}(x_t-\mu) = 0 \quad (1)\]

The only unique solution to the equation is the median.
...
for (i in 1:9) {
  cat("20 Samples Type ", i, " Median: ", quantile(laplace_data, 0.5, type = i), "\n")
}
...
for (i in 1:9) {
  cat("1000 Samples Type ", i, " Median: ", quantile(laplace_data, 0.5, type = i), "\n")
}
